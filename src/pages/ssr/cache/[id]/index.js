...
export const getServerSideProps = async ({ res, query }) => {
  ...
  console.log(`[Server]: Cache ${query.id} trigger (${date})`);
...
export default function SsrPage(props) {
  ...
  return (
    <main>
      <span className="fixed top-0 left-0 w-full h-full bg-green-500/30 pointer-events-none -z-10"></span>
      ...
      <div className="p-6 grid gap-6">
        <Card className="sticky top-0 left-0 z-50 bg-muted">
          <CardHeader>
            <CardTitle>SSR (Cache) {router.query.id}</CardTitle>
          </CardHeader>
          <CardContent>
            <CardTitle>
